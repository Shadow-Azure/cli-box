...
- 字体：嵌入一个 **CJK-capable 等宽 TTF**（`include_bytes!`，如 Sarasa Mono SC 或 Noto Sans Mono CJK，~5–20MB），保证云端环境一致且**中文正确渲染**（不依赖系统字体）。理由：中文环境下 CLI 输出含 CJK，豆腐块会使自动化反馈失效。
...
2. **嵌入式字体体积**：选定 **CJK 等宽字体**（~5–20MB），二进制明显增大。缓解：后续可用字体子集化（`pyftsubset` 按实际字形裁剪）或按需落盘到 `~/.cli-box/` 首次运行加载。
...
| 字体资源（如 `assets/fonts/`） | **新增** 嵌入式 CJK 等宽 TTF（Sarasa/Noto Mono CJK） |
